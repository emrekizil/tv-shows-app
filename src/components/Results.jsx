...
const Results = ({data,setData}) => {
    ...
    return (  
        <div className="shows  grid gap-4 sm:grid-cols-2 md:grid-cols-3">
            {data.map((shows)=>(
                <div className="bg-f1 py-6 px-4 drop-shadow-sm hover:bg-f4  inline-block transform transition hover:-translate-y-4 " key={shows.show.id}>
                    <div className="flex justify-center">
                    {shows.show.image && <img src={shows.show.image.medium} className="mb-5" alt="" />}
                    </div>
                    <h2 className="mb-5 h-12 text-f2">{shows.show.name}</h2>
